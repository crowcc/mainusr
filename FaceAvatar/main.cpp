...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow){
	...
	//缓存背景图片
	pCapture = cvCaptureFromFile("http://192.168.11.6:8080/?action=stream&amp;amp;type=.mjpg");//webcamera
...
#pragma region 骨骼相关
	...
	LeftElbowFrameNo2 = MV1SearchFrame(ModelHandleLen, "左手捩1");
	RightElbowFrameNo3 = MV1SearchFrame(ModelHandleLen, "右手捩2");
	LeftElbowFrameNo3 = MV1SearchFrame(ModelHandleLen, "左手捩2");
	RightElbowFrameNo4 = MV1SearchFrame(ModelHandleLen, "右手捩3");
	LeftElbowFrameNo4 = MV1SearchFrame(ModelHandleLen, "左手捩3");
	RightElbowFrameNo5 = MV1SearchFrame(ModelHandleLen, "右手捩4");
	LeftElbowFrameNo5 = MV1SearchFrame(ModelHandleLen, "左手捩4");

	RightElbowFrameNo6 = MV1SearchFrame(ModelHandleLen, "右袖連動");
	LeftElbowFrameNo6 = MV1SearchFrame(ModelHandleLen, "左袖連動");*/
	...
	//MV1ResetFrameUserLocalMatrix(ModelHandleLen, RightShoulderFrameNo);
	...
	/*LocalPositionRE2 = NOtoVector(ModelHandleLen, RightElbowFrameNo2);
	LocalPositionLE2 = NOtoVector(ModelHandleLen, RightElbowFrameNo2);
	LocalPositionRE3 = NOtoVector(ModelHandleLen, RightElbowFrameNo3);
	LocalPositionLE3 = NOtoVector(ModelHandleLen, LeftElbowFrameNo3);
	LocalPositionRE4 = NOtoVector(ModelHandleLen, RightElbowFrameNo4);
	LocalPositionLE4 = NOtoVector(ModelHandleLen, LeftElbowFrameNo4);
	LocalPositionRE5 = NOtoVector(ModelHandleLen, RightElbowFrameNo5);
	LocalPositionLE5 = NOtoVector(ModelHandleLen, LeftElbowFrameNo5);
	LocalPositionRE6 = NOtoVector(ModelHandleLen, RightElbowFrameNo6);
	LocalPositionLE6 = NOtoVector(ModelHandleLen, LeftElbowFrameNo6);*/

	//testm1 = MV1GetFrameBaseLocalMatrix(ModelHandleLen, RightShoulderFrameNo);//取得初期变换矩阵

	//MV1SetFrameUserLocalMatrix(ModelHandleLen, RightShoulderFrameNo, testm1);	//指定のフレームの座標変換行列を設定する
#pragma endregion
...
#pragma region 手柄控制
		...
		if (joyz != 0 || joydz != 0){        //手柄有输入时才根据输入改相机坐标系下坐标
			...
			lenwx = lenwx - sin(DX_PI_F*(lenro) / 180); //
			lenwz = lenwz - cos(DX_PI_F*(lenro) / 180);
			lenW = { lenwx, 0, lenwz };
